...
class VisualizationSuite:
    ...
    def figure8_integrated_spatial_distribution(self):
        ...
        for target_receptor_idx in target_indices:
            if target_receptor_idx in receptors_df_analyzed.index:
                receptor_info = receptors_df_analyzed.loc[target_receptor_idx]
        ...
        for target_receptor_idx in target_indices:
            if target_receptor_idx in receptors_df_analyzed.index:
                receptor_info = receptors_df_analyzed.loc[target_receptor_idx]
                # Use receptor_sample (original name like R-22) instead of index
                receptor_label = receptor_info.get('receptor_sample', f'R{target_receptor_idx}')
                # Reduced size (400->250) and increased transparency (0.9->0.6) to avoid covering nearby sources
                ax.scatter(receptor_info['lon'], receptor_info['lat'],
                          c='red', s=250, marker='*',
                          edgecolors='black', linewidth=1.5,
                          alpha=0.6, zorder=5)
                ax.annotate(receptor_label,
                           (receptor_info['lon'], receptor_info['lat']),
                           xytext=(8, 8), textcoords='offset points',
                           fontsize=11, fontweight='bold',
                           bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.7),  # Slightly more opaque label box for readability
                           zorder=6)
